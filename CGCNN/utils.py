...
def get_mini_batch(x_signal,graph,y, start, end):
    ...
    for i in range(start,end):
        ...
        adj.append(temp)
    adj = np.array(adj)
    # print('1111111111111111',adj.shape)
    adj = adj[:,0,:]
    return x_signal[start:end],adj,y[start:end]

def add_noise(batch_data,sigma,clip):
...
def weight_dict_fc(trainLabel, para):
    ...
    y_total_40=label_binarize(train_labels, classes=[i for i in range(para.outputClassN)]) #one_hot编码（但前面好像已经编码过了？）
    class_distribution_40_class=np.sum(y_total_40,axis=0) #每个类分别有多少样本
    class_distribution_40_class=[float(i) for i in class_distribution_40_class]
    class_distribution_40_class=class_distribution_40_class/np.sum(class_distribution_40_class) #每个类在总样本中出现的概率
...
def weights_calculation(batch_labels,weight_dict):
    weights = []
    batch_labels = np.argmax(batch_labels,axis=1)#将标签转为（0，1，1，2）这种形式
...
def uniform_weight(trainLabel):
    ...
    # weights = scaled_laplacian(weights)
